...
Tabelul de legătură (bridge) permite relația many-to-many între tichete și tag-uri în depozit: un ticket poate avea mai multe tag-uri, iar un tag poate fi asociat la mai multe tichete. Este populat de scriptul ETL \texttt{04\_load\_bridge.sql}, după ce \texttt{FACT\_TICKET} și \texttt{DIM\_TAG} sunt deja populate.
...
Dimensiunea temporală este populată prin scriptul ETL \texttt{01\_load\_dim\_time.sql}, care generează toate zilele dintr-un interval (ex. 2020--2030). Câmpurile sunt calculate pe baza datei (\texttt{curr\_date}):
...
\hline
\ttwrap{este\_sarbatoare} & CHAR(1) & Opțional în schema; poate fi populat cu logică de sărbători legale \\
\hline
\ttwrap{nume\_sarbatoare} & VARCHAR2 & Opțional; numele sărbătorii dacă există \\
\hline
\ttwrap{zi\_lucratoare} & CHAR(1) & Opțional; 'Y'/'N' în funcție de weekend/sărbătoare \\
\hline
\end{longtable}
...
Toate tabelele dimensiune și tabelul de fapte folosesc constrângeri de cheie străină (FK). În implementare, FK-urile din \texttt{FACT\_TICKET} și \texttt{BRIDGE\_TICKET\_TAG} sunt definite cu \textbf{RELY DISABLE NOVALIDATE}.

\begin{itemize}
    \item \textbf{FK\_FACT\_CLIENT}: \texttt{FACT\_TICKET.client\_key} $\rightarrow$ \texttt{DIM\_CLIENT.client\_key}
    \item \textbf{FK\_FACT\_AGENT}: \texttt{FACT\_TICKET.agent\_key} $\rightarrow$ \texttt{DIM\_AGENT.agent\_key}
    \item \textbf{FK\_FACT\_DEPT}: \texttt{FACT\_TICKET.departament\_key} $\rightarrow$ \texttt{DIM\_DEPARTAMENT.departament\_key}
    \item \textbf{FK\_FACT\_CAT}: \texttt{FACT\_TICKET.categorie\_key} $\rightarrow$ \texttt{DIM\_CATEGORIE.categorie\_key} (poate fi NULL)
    \item \textbf{FK\_FACT\_TOPIC}: \texttt{FACT\_TICKET.topic\_key} $\rightarrow$ \texttt{DIM\_TOPIC.topic\_key}
    \item \textbf{FK\_FACT\_DT\_CR}: \texttt{FACT\_TICKET.date\_creare\_key} $\rightarrow$ \texttt{DIM\_TIME.date\_key}
    \item \textbf{FK\_BRIDGE\_FACT} / \textbf{FK\_BRIDGE\_DIM}: în \texttt{BRIDGE\_TICKET\_TAG}, legături către \texttt{FACT\_TICKET} și \texttt{DIM\_TAG}
\end{itemize}

\subsubsection{RELY DISABLE NOVALIDATE}

FK-urile de la \texttt{FACT\_TICKET} și \texttt{BRIDGE\_TICKET\_TAG} către dimensiuni sunt create cu clauzele \texttt{RELY}, \texttt{DISABLE} și \texttt{NOVALIDATE}:
...
\begin{itemize}
    ...
    \end{itemize}
    \item \textbf{CHECK pentru FACT\_TICKET}: (dacă există câmp \texttt{rating\_feedback}, între 1 și 5; în implementarea curentă nu e prezent în fact.)
\end{itemize}
...
\subsection{Tipuri de partiționare}

\begin{itemize}
    \item \textbf{Partiționare RANGE} pe \texttt{date\_creare\_key}: partiții pe ani (2023, 2024, \ldots, 2030) plus \texttt{fact\_tickets\_viitor} (\texttt{MAXVALUE}).
    \item \textbf{Subpartiționare HASH} pe \texttt{topic\_key} în 4 subpartiții per partiție.
\end{itemize}

\subsection{Partiții definite}

Partițiile RANGE sunt: \texttt{fact\_tickets\_2023} (VALUES LESS THAN 20240000), \texttt{fact\_tickets\_2024} (20250000), \ldots, \texttt{fact\_tickets\_2030} (20310000), \texttt{fact\_tickets\_viitor} (MAXVALUE).

\section{Cereri în limbaj natural}

\textbf{Cerere în limbaj natural nr 1:} „Să se calculeze numărul de tichete și timpul mediu de rezolvare (în ore) pentru tichetele create în anul 2024, grupate pe departament și pe status.”

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{../../scripts/tests/assets/partitie_range.png}
    \caption{Partiționare RANGE}
    \label{fig:partitie_range}
\end{figure}

\textbf{Cerere în limbaj natural nr 2:} „Analiza performanței pentru un anumit Topic critic (ex: ID 105) în anul 2024.”

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{../../scripts/tests/assets/partitie_range_hash.png}
    \caption{Partiționare RANGE și HASH}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \textbf{Cerere în limbaj natural:} „Să se afișeze datele (identificator, CNP, nume, prenume) persoanelor fizice care au numele egal cu «RADU»”
    \end{itemize}
    ...
    \begin{itemize}
        \item \textbf{Cerere în limbaj natural:} „Să se afișeze identificatorul, titlul, statusul și data creării pentru toate tichetele create în anul 2024, între luna februarie și luna decembrie.”
    \end{itemize}
    ...
    \begin{itemize}
        \item \textbf{Cerere în limbaj natural:} „Să se afișeze identificatorul din OLAP, cheia clientului și timpul de rezolvare (în ore) pentru tichetele care au un anumit status și/sau o anumită prioritate.”
    \end{itemize}
