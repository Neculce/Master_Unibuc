...
Definiți în \texttt{03\_create\_oltp\_indexes.sql}:
...
Definiți în \texttt{07\_create\_olap\_indexes.sql}:
...
\section{Cereri SQL și Plan de Execuție (Utilizarea Indecșilor)}

Pentru demonstrarea utilizării indecșilor, în \texttt{scripts/tests/06\_indexes.sql} sunt definite cereri SQL însoțite de \texttt{EXPLAIN PLAN} și \texttt{DBMS\_XPLAN.DISPLAY}:

\begin{enumerate}
    \item \textbf{OLTP — index function-based pe \texttt{client\_fizica}}: cerere care filtrează pe \texttt{UPPER(nume) = 'RADU'}; planul trebuie să indice utilizarea \texttt{idx\_client\_nume\_upper}.
    \item \textbf{OLTP — index pe \texttt{ticket.data\_creare}}: cerere care filtrează pe interval de date (\texttt{data\_creare} între feb.–dec. 2024); planul trebuie să indice \texttt{idx\_ticket\_data\_creare}.
    \item \textbf{Depozit — indecși bitmap pe \texttt{fact\_ticket}}: cerere cu hint \texttt{INDEX\_COMBINE} care filtrează pe \texttt{status\_id} și \texttt{prioritate\_id}; planul trebuie să indice utilizarea \texttt{bidx\_fact\_status} și \texttt{bidx\_fact\_prioritate}.
\end{enumerate}

Rezultatele planurilor de execuție sunt în \texttt{scripts/tests/assets/} și incluse în documentație pentru a dovedi că optimizorul folosește indecșii definiți.
